...
const ALWAYS_FORBIDDEN = [".github/**", "**/package.json"];
// Forbidden too, except on the bot's own branch: reconciling the lockfile with someone else's bump
// is the repair's whole job, and `npm ci` must succeed from clean once it is done. `package.json`
// above is deliberately not exempted -- which version to declare is the bot's call, not the agent's.
const FORBIDDEN_UNLESS_REPAIRING = ["**/package-lock.json"];
...
// The agent's commit riding on the dependency bot's branch: the bump is in the same range, and it
// is what the tests are proving.
export const isRepair = (changes: Change[], config: Config): boolean => changes.some((c) => c.author === config.botAuthor);
...
export function violations(changes: Change[], config: Config): string[] {
  ...
  const out: string[] = [];
  // The caller's list is appended last, so a caller that forbids the lockfile itself still wins.
  const forbidden = [...ALWAYS_FORBIDDEN, ...(repairing ? [] : FORBIDDEN_UNLESS_REPAIRING), ...config.forbidden];
  ...
  // A lockfile regenerated by the package manager runs to thousands of lines that are not the
  // agent's authorship. Counting them would refuse every repair the exemption above just allowed.
  const counted = repairing ? agent.filter((c) => !matchesAny(c.path, FORBIDDEN_UNLESS_REPAIRING)) : agent;
  ...
  if (agent.length && !repairing && !testsToProve(changes, config).length) out.push(`${config.agentAuthor} fix adds no unit test`);
  return out;
}
